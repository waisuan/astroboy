...
func main() {
	...
	for i, g := range strings.Split(input(), "\n") {
		matrix[i] = make([]string, len(g))
		for j, g2 := range g {
			matrix[i][j] = string(g2)
		}
	}
	...
	for i, row := range matrix {
		for j, c := range row {
			...
			_, ok := tracker[fmt.Sprintf("%d,%d", i, j)]
			if ok {
				continue
			}
			...
			if unicode.IsDigit(col) {
				...
				if i-1 >= 0 {
					if matrix[i-1][j] != "." && !unicode.IsDigit([]rune(matrix[i-1][j])[0]) {
						symbolDetected = true
					}
					if j-1 >= 0 && matrix[i-1][j-1] != "." && !unicode.IsDigit([]rune(matrix[i-1][j-1])[0]) {
						symbolDetected = true
					}
					if j+1 < len(row) && matrix[i-1][j+1] != "." && !unicode.IsDigit([]rune(matrix[i-1][j+1])[0]) {
						symbolDetected = true
					}
				}
				if i+1 < len(row) {
				...
				if j-1 >= 0 && matrix[i][j-1] != "." && !unicode.IsDigit([]rune(matrix[i][j-1])[0]) {
					symbolDetected = true
				}
				if j+1 < len(row) && matrix[i][j+1] != "." && !unicode.IsDigit([]rune(matrix[i][j+1])[0]) {
					symbolDetected = true
				}
				...
				for k := j + 1; k < len(row); k++ {
					...
					if unicode.IsDigit(tmp) {
						...
						if !symbolDetected {
							if i-1 >= 0 {
								if matrix[i-1][k] != "." && !unicode.IsDigit([]rune(matrix[i-1][k])[0]) {
									symbolDetected = true
								}
								if k-1 >= 0 && matrix[i-1][k-1] != "." && !unicode.IsDigit([]rune(matrix[i-1][k-1])[0]) {
									symbolDetected = true
								}
								if k+1 < len(row) && matrix[i-1][k+1] != "." && !unicode.IsDigit([]rune(matrix[i-1][k+1])[0]) {
									symbolDetected = true
								}
							}
							if i+1 < len(row) {
								...
								if j-1 >= 0 && matrix[i+1][k-1] != "." && !unicode.IsDigit([]rune(matrix[i+1][k-1])[0]) {
									symbolDetected = true
								}
								if k+1 < len(row) && matrix[i+1][k+1] != "." && !unicode.IsDigit([]rune(matrix[i+1][k+1])[0]) {
									symbolDetected = true
							...
							if k+1 < len(row) && matrix[i][k+1] != "." && !unicode.IsDigit([]rune(matrix[i][k+1])[0]) {
								symbolDetected = true
							}
						}

						tracker[fmt.Sprintf("%d,%d", i, k)] = true
					} else {
						break
					}
				}
